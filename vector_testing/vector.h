...
class vector {
    ...
    void push_back_long(T const &elem) {
        if (aux_size() < aux_capacity()) {
            ...
            aux* new_aux = copy(aux_capacity() * 2);
            try {
                construct(new_aux->get_ptr() + size(), elem);
                ++aux_ptr()->size;
            } catch (...) {
...
public:
    ...
    void clear() {
        assert(small() || aux_ptr() != nullptr);
        if (!small() && aux_ptr()->count == 0) {
            for (size_t i = 0; i < size(); ++i) {
                deconstruct(aux_ptr()->get_ptr() + i);
            }
            dealloc(aux_ptr());
        } else if (!small() && aux_ptr()->count != 0) {
            --aux_ptr()->count;
        }
        data_ = nullptr;
    }
